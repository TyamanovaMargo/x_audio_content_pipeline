...
def main(input_file, force_recheck=False):
    """Main pipeline execution - YouTube & Twitch Voice Content Pipeline (8 stages with noise reduction)"""
    cfg = Config()
    os.makedirs(cfg.OUTPUT_DIR, exist_ok=True)
    ...
    print("=" * 60)
    print("🎯 Focus: YouTube and Twitch voice content extraction")
    print("🎤 Output: 30-second voice samples -> noise reduction -> voice-only filtering")
...
def show_help():
    """Show detailed usage help"""
    help_text = """
🎙️ YOUTUBE & TWITCH VOICE CONTENT PIPELINE (8-Stage Processing with Noise Reduction)

This pipeline validates X/Twitter accounts, collects profile data through Bright Data API,
extracts external links, filters for YouTube & Twitch platforms, detects audio content,
verifies voice/speech content, extracts 30-second voice samples, applies noise reduction,
and filters for voice-only content.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="YouTube & Twitch Voice Content Pipeline with 8-Stage Processing (Noise Reduction included)",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
